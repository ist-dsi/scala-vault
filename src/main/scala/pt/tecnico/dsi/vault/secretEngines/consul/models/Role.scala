...
object Role {
  ...
  implicit val encoder: Encoder[Role] = deriveEncoder[Role](renaming.snakeCase).contramap(r => r.copy(policy = base64encode(r.policy)))
  implicit val decoder: Decoder[Role] = deriveDecoder[Role](renaming.snakeCase).map(r => r.copy(policy = base64decode(r.policy)))
}
