...
import pt.tecnico.dsi.vault.models.{DefaultService, TokenType}

object TokenRole {
  implicit val decoder: Decoder[TokenRole] = deriveDecoder
  implicit val encoder: Encoder[TokenRole] = deriveEncoder
}
/**
  ...
  */
case class TokenRole(name: String, orphan: Boolean = false,
                     allowedPolicies: List[String] = List.empty, disallowedPolicies: List[String] = List.empty,
                     explicitMaxTtl: Int = 0, period: Int = 0, renewable: Boolean = true,
