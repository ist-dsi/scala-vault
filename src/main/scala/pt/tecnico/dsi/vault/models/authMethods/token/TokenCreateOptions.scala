...
object TokenCreateOptions {
  implicit val decoder: Decoder[TokenCreateOptions] = deriveDecoder
  implicit val encoder: Encoder[TokenCreateOptions] = deriveEncoder
}
/**
  ...
  */
case class TokenCreateOptions(displayName: String = "token", policies: List[String] = List.empty, noDefaultPolicy: Boolean = false,
                              roleName: String = "", noParent: Boolean = false, numUses: Int = 0,
                              renewable: Boolean = true, ttl: String = "", explicitMaxTtl: String = "", period: String = "",
                              id: Option[String] = None, meta: Map[String, String] = Map.empty)
